	...
	if #clean >= 3 and clean:match("%d") then
		...
		if func then
			...
			if success and type(result) == "number" then
				say("The answer to " .. clean .. " is " .. tostring(result))
			end
		end
	end
end)
		...
		while active and hum and hum.Health > 0 do
			...
			if target and not resting and not threatPart then
				...
				if stareTimer < maxStare then
					...
					if tick() - lastSay > 15 then say(behaviorConfig[behaviorName].Lines[math.random(1, #behaviorConfig[behaviorName].Lines)], target.Name) lastSay = tick() end
...
local function setup(c)
	...
	say("Systems online. Intelligence module active.")
	hum.HealthChanged:Connect(function(h)
		if h < lastHealth and h > 0 then
			if not threatPart then
				say("Damage detected! Initiating evasive maneuvers!")
	...
	hum.Died:Connect(function()
		say("NPC has deceased. Disabling systems.")
		active = false tglB.Text, tglB.BackgroundColor3 = "START AI", Color3.new(0,0.6,0)
		if statThread then task.cancel(statThread) task.cancel(pathThread) task.cancel(lookThread) end
	end)
...
if player.Character then setup(player.Character) endlocal ui = Instance.new("ScreenGui", player.PlayerGui)
ui.Name = "GTANPC_ULTIMATE_FIX"
ui.ResetOnSpawn = false
...
Chat.MessageReceived:Connect(function(textMsg)
	...
	if #clean >= 3 and clean:match("%d") then
		...
		if func then
			local success, result = pcall(func)
			if success and type(result) == "number" then
				say("Calculated " .. clean .. ". Result: " .. tostring(result))
			end
		end
	end
end)
		...
		while active and hum and hum.Health > 0 do
			...
			if target and not resting then
				...
				if stareTimer < maxStare then
					...
					root.CFrame = root.CFrame:Lerp(CFrame.lookAt(root.Position, lookPos), 0.15)
					if tick() - lastSay > 15 then say(behaviorConfig[behaviorName].Lines[math.random(1, #behaviorConfig[behaviorName].Lines)], target.Name) lastSay = tick() end
				else hum.AutoRotate = true stareTimer, cooldown = 0, (behaviorName == "Observant") and 5 or 12 end
			else hum.AutoRotate = true stareTimer = 0 end
			task.wait(0.1)
		end
	end)
...
local function setup(c)
	...
	hum.Seated:Connect(function(s) resting = s end)
	hum.UseJumpPower, lastHealth = true, hum.Health
	say("NPC Rebooted. Systems online.")
	hum.Died:Connect(function()
		say("Critical failure. Shutting down...")
		active = false tglB.Text, tglB.BackgroundColor3 = "START AI", Color3.new(0,0.6,0)
		if statThread then task.cancel(statThread) task.cancel(pathThread) task.cancel(lookThread) end
	end)
	hum.StateChanged:Connect(function(_, state)
...
player.CharacterAdded:Connect(setup)
if player.Character then setup(player.Character) endak end
					if (root.Position - wp.Position).Magnitude > 350 then break end
					hum:MoveTo(wp.Position)
					if wp.Action == Enum.PathWaypointAction.Jump then hum.Jump = true end
					local ray = Ray.new(root.Position, root.CFrame.LookVector * 5)
